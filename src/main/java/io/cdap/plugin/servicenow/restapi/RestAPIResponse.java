...
public class RestAPIResponse {
  ...
  private static final String HTTP_ERROR_MESSAGE =
      "Http call to ServiceNow instance returned status code %d.";
  private static final String REST_ERROR_MESSAGE =
      "Rest Api response has errors. Error message: %s.";
  private static final Set<Integer> SUCCESS_CODES =
      new HashSet<>(Collections.singletonList(HttpStatus.SC_OK));
  ...
  /**
   * Parses HttpResponse into RestAPIResponse object when no errors occur. Throws a {@link
   * RetryableException} if the error is retryable. Throws an {@link NonRetryableException} if the
  ...
  public static RestAPIResponse parse(HttpResponse httpResponse, String... headerNames) {
    ...
    try {
      ...
      return new RestAPIResponse(headers, null, new ServiceNowAPIException(e, httpResponse, false));
    }
  ...
  private static ServiceNowAPIException validateRestApiResponse(
    ...
    if (errorMessage.contains(ServiceNowConstants.MAXIMUM_EXECUTION_TIME_EXCEEDED)) {
      return new ServiceNowAPIException(
          String.format(REST_ERROR_MESSAGE, errorMessage), response, true);
    } else {
      return new ServiceNowAPIException(
          String.format(REST_ERROR_MESSAGE, errorMessage), response, false);
    }
  }
  ...
  private static ServiceNowAPIException validateHttpResponse(HttpResponse response) {
    ...
    if (RETRYABLE_CODES.contains(code)) {
      return new ServiceNowAPIException(String.format(HTTP_ERROR_MESSAGE, code), response, true);
    }
    return new ServiceNowAPIException(String.format(HTTP_ERROR_MESSAGE, code), response, false);
  }
